...
class czhGSVPano:
    ...
    def getPanoTiles(self):
        ...
        for y in range(h):
            for x in range(w):
                ...
                url = "http://maps.google.com/cbk?output=tile&panoid=" + self.pano_location_prop[
                    'panoId'] + "&zoom=" + str(self.pano_zoom) + "&x=" + \
                      str(x) + "&y=" + str(y)
    ...
    def constructPanoImage(self, save_flag):
        ...
        for pano_tile in self.panoTiles:
            self.panorama.paste(pano_tile['image'], (TILE_SIZE * pano_tile['x'], TILE_SIZE * pano_tile['y'],
                                                     TILE_SIZE * (pano_tile['x'] + 1),
                                                     TILE_SIZE * (pano_tile['y'] + 1)))
    ...
    def outputPanoImage(self, img, saved_path="", prefix="", suffix=""):
        ...
        file_name = os.path.join(saved_path, (prefix + self.pano_location_prop['panoId'] + '_' +
                                              str(self.pano_location_prop['lat']) + '_' + str(
                    self.pano_location_prop['lng']) + suffix + '.jpg'))
        img.save(file_name)
...
class czhGSVDepthmap:
    ...
    def outputDepthmapJson(self, depthmap_json_data, saved_path='', prefix='', suffix=''):
        ...
        if depthmap_json_data is not None:
            json_file_name = os.path.join(saved_path, prefix + depthmap_json_data['Location']['lng'] + '_' +
                                          depthmap_json_data['Location']['lat'] + '_' + depthmap_json_data['Location'][
                                              'panoId'] + suffix + '.json')
    ...
    def outputSourceDepthmap2Image(self, depthmap_source_data, saved_path='', prefix='', suffix='', ext=''):
        ...
        file_name = os.path.join(saved_path, (prefix + self.json_data['Location']['panoId'] + '_' +
                                              str(self.json_data['Location']['lat']) + '_' + str(
                    self.json_data['Location']['lng']) + suffix + ext))
...
class czhGSVPointClouds():
    ...
    def __init__(self, panorama: czhGSVPano, depthmap: czhGSVDepthmap, save_xyz_flag: int = 0,
                 save_pointclouds2img_flag: int = 1, resolution: float = 0.1):
    ...
    def pointCloud2Image(self, rawPointsCloud, resolution=0.25, saved_path='', prefix='', suffix='', ext=''):
        ...
        imgfile_name = os.path.join(saved_path, (prefix + self.c_panorama.pano_location_prop['panoId'] + '_' +
                                                 str(self.c_panorama.pano_location_prop['lat']) + '_' + str(
                    self.c_panorama.pano_location_prop['lng']) + suffix + ext))
        # save image
    ...
    def constructRawPointsCloud2(self, fov_h, theta0, phi0, w_thumb, h_thumb):
        ...
        nImage, pano_thetas, pano_phis = self.clip_pano2(theta, phi, 0, 0, math.radians(fov_h), math.radians(fov_v),
        ...
        ndepthmap_clip = self.clip_pano2(theta, phi, 0, 0, math.radians(fov_h), math.radians(fov_v),
                                         depth_width * w_thumb / pano_width, ndepthmap.tolist())
        ...
        try:
            ...
            for y in range(h_thumb):
                ...
                for x in range(w_thumb):
                    ...
                    if 0.0005 < depth < 20.0:  # and color_value ==6
